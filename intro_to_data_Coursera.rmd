...
sfo_feb_flights %>% 
  ggplot() + geom_histogram(aes(x=arr_delay), bins=20) 

sfo_feb_flights %>% 
  ggplot() + geom_boxplot(aes(y=arr_delay))
...
sfo_feb_flights %>% group_by(carrier) %>% summarise(median(arr_delay), IQR(arr_delay))
```
...
nycflights %>% 
  mutate(avg_speed = distance / air_time * 60) %>% 
  arrange(desc(avg_speed)) %>% 
  ggplot() + geom_point(aes(x = distance, y = avg_speed)) 
...
flights_delay_type <- nycflights %>% 
  mutate(arr_type = case_when(arr_delay > 0 ~ "arr delayed", 
                              TRUE ~ "arr on time"),
         dep_type = case_when(dep_delay > 0 ~ "dep delayed", 
                              TRUE ~ "dep on time")) %>% 
  select(arr_type, dep_type) %T>% print

print(nrow(flights_delay_type))


table(flights_delay_type$arr_type, flights_delay_type$dep_type) / nrow(flights_delay_type)
